...
--
--  IT DOES NOT CARE WHAT WAS DONE IN THE GUI BEFORE
--  Right after START TRANSACTION, t_fixture() replaces the rows of every
--  table with the seed data of the report (the rows of the demo state, taken
--  from baseisproject_dump.sql). So the checks always start from the same
--  data - the 500 EUR adult price of trip 1, bus 9 free at 90 000 km - even
--  after a GUI demo has booked hotels or put bus 9 on a new trip. The
--  procedures and triggers are the real ones; only the rows are reset, and
--  the ROLLBACK puts the GUI's rows back exactly as they were.
--
...
-- =====================================================================
-- @log_before counts the rows, @log_last is the newest id: after rows have been
-- deleted the two differ, so the id is used to pick out the new rows
SET @log_before = (SELECT COUNT(*) FROM log_actions);
SET @log_last   = (SELECT IFNULL(MAX(log_id), 0) FROM log_actions);
...
CALL t_eq('12. 3.1.4.1 audit log', 'customer logs INSERT, UPDATE and DELETE',
    (SELECT GROUP_CONCAT(DISTINCT log_action_type ORDER BY log_action_type) FROM log_actions
     WHERE log_table_name = 'customer' AND log_id > @log_last), 'INSERT,UPDATE,DELETE');
CALL t_eq('12. 3.1.4.1 audit log', 'destination logs INSERT, UPDATE and DELETE',
    (SELECT GROUP_CONCAT(DISTINCT log_action_type ORDER BY log_action_type) FROM log_actions
     WHERE log_table_name = 'destination' AND log_id > @log_last), 'INSERT,UPDATE,DELETE');
CALL t_eq('12. 3.1.4.1 audit log', 'vehicle logs INSERT, UPDATE and DELETE',
    (SELECT GROUP_CONCAT(DISTINCT log_action_type ORDER BY log_action_type) FROM log_actions
     WHERE log_table_name = 'vehicle' AND log_id > @log_last), 'INSERT,UPDATE,DELETE');
CALL t_eq('12. 3.1.4.1 audit log', 'lodging logs INSERT, UPDATE and DELETE',
    (SELECT GROUP_CONCAT(DISTINCT log_action_type ORDER BY log_action_type) FROM log_actions
     WHERE log_table_name = 'lodging' AND log_id > @log_last), 'INSERT,UPDATE,DELETE');
CALL t_eq('12. 3.1.4.1 audit log', 'room_usage logs INSERT, UPDATE and DELETE',
    (SELECT GROUP_CONCAT(DISTINCT log_action_type ORDER BY log_action_type) FROM log_actions
     WHERE log_table_name = 'room_usage' AND log_id > @log_last), 'INSERT,UPDATE,DELETE');
CALL t_eq('12. 3.1.4.1 audit log', 'reservation logs INSERT, UPDATE and DELETE',
    (SELECT GROUP_CONCAT(DISTINCT log_action_type ORDER BY log_action_type) FROM log_actions
     WHERE log_table_name = 'reservation' AND log_id > @log_last), 'INSERT,UPDATE,DELETE');
CALL t_eq('12. 3.1.4.1 audit log', 'trip logs INSERT, UPDATE and DELETE',
    (SELECT GROUP_CONCAT(DISTINCT log_action_type ORDER BY log_action_type) FROM log_actions
     WHERE log_table_name = 'trip' AND log_id > @log_last), 'INSERT,UPDATE,DELETE');

CALL t_eq('12. 3.1.4.1 audit log', 'all seven tables of the requirement are covered',
    (SELECT COUNT(DISTINCT log_table_name) FROM log_actions WHERE log_id > @log_last), '7');
CALL t_eq('12. 3.1.4.1 audit log', 'the account that made the change is recorded',
    (SELECT COUNT(DISTINCT log_dba_username) FROM log_actions WHERE log_id > @log_last), '1');
CALL t_eq('12. 3.1.4.1 audit log', 'the account is stored without the host name',
    (SELECT DISTINCT log_dba_username FROM log_actions WHERE log_id > @log_last),
...
CALL t_has('12. 3.1.4.1 audit log', 'the log says what changed, not only that something did',
    (SELECT log_details FROM log_actions
     WHERE log_table_name = 'vehicle' AND log_action_type = 'UPDATE' AND log_id > @log_last
