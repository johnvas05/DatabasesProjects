-- =====================================================================
-- Reset.sql - put the database back into the demo state
-- =====================================================================
-- Run this before (or between) presentations. It restores every table to
-- the seed data of the report and wipes everything a demo produces, so the
-- application always starts from the same, known state.
--
--   * base tables (branch, worker, driver, guide, admin, customer,
--     destination, trip, travel_to, event, reservation, vehicle, lodging)
--     are emptied and re-seeded with the exact data of queries/Insertions.sql
--   * room_usage is emptied, so "Auto-Book Accommodations" can be shown again
--   * log_actions is emptied LAST, so the Admin -> System Logs tab starts
--     clean and shows only the actions performed during the demo
--   * AUTO_INCREMENT counters go back to 1 (trip 1..14, lodging 1..10, ...),
--     so the ids quoted in the report and in the tests stay correct
--   * trip_history (90 000 rows, 3.1.2.3) is NOT touched - regenerating it
--     takes minutes and its content never changes during a demo
--
-- Usage
--   docker exec -i baseis-mariadb mariadb -uroot -pjohn2005 baseisproject < queries/Reset.sql
--   (or simply:  tests/reset_db.sh)
--
-- The seed part below is identical to queries/Insertions.sql, except that the
-- customer birth dates are computed from cust_id instead of RAND(), so the
-- adult/child pricing demo gives the same numbers every time.
-- =====================================================================
...
UPDATE customer
SET cust_birth_date = DATE_ADD('1980-01-01', INTERVAL (cust_id * 211) MOD 3650 DAY)
WHERE cust_id <= 15;

-- We will make the last 5 customers Children (born 2015-2020)
-- This ensures you can test "Child Pricing" logic later
UPDATE customer
SET cust_birth_date = DATE_ADD('2015-01-01', INTERVAL (cust_id * 97) MOD 1800 DAY)
WHERE cust_id > 15;
...
-- =====================================================================
-- Price every reservation, the way the GUI does it after a booking
-- (sp_calculate_reservation_cost): the adult price of the trip, or the child
-- price when the customer is under 18. The seed inserts the reservations
-- without a price, and a price of 0 would make every branch look unprofitable
-- (Admin -> Branch Financials, and the salary trigger that uses it).
-- =====================================================================
...
-- =====================================================================
-- Clear the audit log LAST: the seed above fires the log triggers (3.1.4.1)
-- on every insert, and that noise should not be in the log the examiner sees.
-- From here on, every row in log_actions comes from the demo itself.
-- =====================================================================
TRUNCATE TABLE log_actions;

-- =====================================================================
-- Report: what the database looks like after the reset
-- =====================================================================
SELECT 'branch' AS table_name, COUNT(*) AS rows_after_reset FROM branch
...
UNION ALL SELECT 'destination',  COUNT(*) FROM destination
UNION ALL SELECT 'trip',         COUNT(*) FROM trip
UNION ALL SELECT 'travel_to',    COUNT(*) FROM travel_to
UNION ALL SELECT 'event',        COUNT(*) FROM event
UNION ALL SELECT 'reservation',  COUNT(*) FROM reservation
UNION ALL SELECT 'reservation (priced)', COUNT(*) FROM reservation WHERE res_total_cost > 0
UNION ALL SELECT 'vehicle',      COUNT(*) FROM vehicle
UNION ALL SELECT 'lodging',      COUNT(*) FROM lodging
UNION ALL SELECT 'room_usage (emptied)',   COUNT(*) FROM room_usage
UNION ALL SELECT 'log_actions (emptied)',  COUNT(*) FROM log_actions
UNION ALL SELECT 'trip_history (kept)',    COUNT(*) FROM trip_history;
